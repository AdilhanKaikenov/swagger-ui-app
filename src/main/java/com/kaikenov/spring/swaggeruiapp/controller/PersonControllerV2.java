...
public class PersonControllerV2 {
    ...
    @GetMapping("/")
    @ApiOperation(value = "This method returns all the Persons in the database with their corresponding Hobbies V2", response = Iterable.class)
    public List<Person> findAll() {
        return personService.findAll();
    }

    @PostMapping("/")
    @ApiOperation(value = "This method can create a new Person and its hobbies if provided V2", response = Person.class)
    public Person create(@RequestBody Person person) {
        return personService.create(person);
    }

    @GetMapping("/{personId}")
    @ApiOperation(value = "This method can find a person with a given ID V2", response = Person.class)
    public Person findOne(@PathVariable("personId") Long personId) {
        return personService.findOne(personId);
    }

    @GetMapping("/find/{firstName}")
    @ApiOperation(value = "This method can find a list of persons by their first name V2", response = Iterable.class)
